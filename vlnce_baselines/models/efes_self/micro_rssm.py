...
class MicroRSSMV2(nn.Module):
    ...
    @staticmethod
    def diag_gaussian_kl(mu_post: Tensor, std_post: Tensor, mu_pri: Tensor, std_pri: Tensor) -> Tensor:
    ...
    def forward(
        ...
        c_micro_kl_raw = self.diag_gaussian_kl(mu_post, std_post, mu_pri, std_pri)
        # Use a compressed non-negative score for diagnosis so early training
        # does not let raw KL dominate every downstream signal.
        c_micro_diag = torch.log1p(c_micro_kl_raw.clamp_min(0.0))
        return {
            "h_t": h_t,
            "mu_pri": mu_pri,
            "std_pri": std_pri,
