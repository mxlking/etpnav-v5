...
stage="${1:-stage1}"
ckpt_path="${2:-}"
exp_prefix="${3:-diag_eval_matrix_r2r_${stage}}"
master_port_base="${4:-29621}"
if [ "$#" -ge 4 ]; then
    shift 4
elif [ "$#" -ge 3 ]; then
    shift 3
...
if [ -z "$ckpt_path" ]; then
    ...
    echo "Example: CUDA_VISIBLE_DEVICES=5 EVAL_ENVS_PER_RANK=1 $0 stage1 data/logs/checkpoints/exp_r2r_v5_s1_4k/best.pth"
    exit 1
fi
...
run_case() {
    ...
    echo "============================================================"
    echo "R2R DIAGNOSTIC CASE   = ${suffix}"
    echo "STAGE                 = ${stage}"
    ...
    failures=$((failures + 1))
    echo "R2R diagnostic case failed: ${suffix}"
    return 0
}
...
echo "Completed diagnostic eval matrix for R2R."
