...
EXP_CONFIG="run_r2r/efes_v3/efes_v3_main.yaml"
STAGE_OPTS=()
case "$stage" in
    main)
        ;;
    short)
        ...
        ;;
    no_predict)
        STAGE_OPTS+=(EFES_V3.use_predictor False)
        ;;
    no_corrector)
        STAGE_OPTS+=(EFES_V3.use_corrector False EFES_V3.action_source etp)
        ;;
    no_somatic)
        STAGE_OPTS+=(EFES_V3.use_somatic_loop False)
        ;;
    *)
        echo "Unknown EFESV3 stage: $stage"
        echo "Valid stages: main short no_predict no_corrector no_somatic"
        exit 1
        ;;
esac
...
COMMON_LOGGING_ARGS=(
    ...
    EFES_V3.LOGGING.step_log_every "$STEP_LOG_EVERY"
    EFES_V3.LOGGING.use_tqdm "$USE_TQDM"
    EFES_V3.LOGGING.write_step_metrics "$WRITE_STEP_METRICS"
    EFES_V3.LOGGING.log_full_model_report_to_runtime "$LOG_FULL_MODEL_REPORT_TO_RUNTIME"
)
...
case "$mode" in
    ...
    eval)
        ...
        echo "[EFESV3 eval] using checkpoint: $CKPT_PATH"
        if [ "$NPROC" -eq 1 ]; then
    ...
    infer)
        ...
        echo "[EFESV3 infer] using checkpoint: $CKPT_PATH"
        if [ "$NPROC" -eq 1 ]; then
