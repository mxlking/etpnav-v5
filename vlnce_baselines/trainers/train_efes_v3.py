...
from vlnce_baselines.agents.efes_v3_agent import EFESV3Agent
from vlnce_baselines.common.utils import extract_instruction_tokens
from vlnce_baselines.datasets.state_label_builder import StateLabelBuilder
from vlnce_baselines.models.efes_v3.topo_bank import TopoBank
...
@baseline_registry.register_trainer(name="EFESV3")
class EFESV3Trainer(EFESSelfTrainer):
    stage_name = "EFESV3"
    contract_version = "v3-clean"
    accepted_contract_versions = ("v3-clean",)
    ...
    def _efes_cfg(self):
        return self.config.EFES_V3

    def _logging_cfg(self):
        return self.config.EFES_V3.LOGGING
    ...
    def _efes_action_source(self) -> str:
        ...
        if source not in {"corrected", "etp"}:
            logger.warning("Unknown EFES_V3.action_source=%s. Falling back to corrected.", source)
            source = "corrected"
        return source
        ...
        payload.update(
            {
                ...
                "use_predictor": bool(getattr(self._efes_cfg(), "use_predictor", True)),
                "use_corrector": bool(getattr(self._efes_cfg(), "use_corrector", True)),
                "use_somatic_loop": bool(getattr(self._efes_cfg(), "use_somatic_loop", True)),
                "alpha": float(getattr(self._efes_cfg(), "alpha", 0.5)),
                "beta": float(getattr(self._efes_cfg(), "beta", 0.1)),
                "lambda_macro": float(getattr(self._efes_cfg(), "lambda_macro", 1.0)),
            }
        )
        return payload
    ...
    def _make_statenav_components(self) -> None:
        ...
        self.statenav_agent = EFESV3Agent.from_config(
            self.config,
            x_dim=etp_dim,
            lang_dim=etp_dim,
            cand_dim=etp_dim,
    ...
    def _configure_trainable_modules(self) -> None:
        ...
        self.optimizer = torch.optim.AdamW(
            [param for param in self.statenav_agent.parameters() if param.requires_grad],
            lr=float(self._efes_cfg().lr),
            weight_decay=0.01,
        )
    ...
    def _load_stage_checkpoint(self, checkpoint_path: str) -> int:
        ...
        if has_statenav_state:
            ...
            if load_msg.missing_keys or load_msg.unexpected_keys:
                raise RuntimeError(
                    "Invalid EFESV3 checkpoint load for {}: missing_keys={}, unexpected_keys={}".format(
                        checkpoint_path,
        ...
        if has_statenav_state:
            logger.info(
                "Loaded EFESV3 checkpoint for %s: %s at iteration %d [contract=%s phase=%s]",
                mode,
                checkpoint_path,
                start_iter,
            ...
            logger.info(
                "Loaded bootstrap-only ETPNav checkpoint for EFESV3 training: %s.",
                checkpoint_path,
            )
        return start_iter
    ...
    def _log_step_metric_record(self, record: Dict[str, Any]) -> None:
        ...
        if int(record["iteration"]) % step_log_every != 0:
            return
        logger.info(
            "[EFESV3 %s action=%s %06d/%06d | interval %03d/%03d] total=%.4f nav=%.4f pred=%.4f cal=%.4f grad=%.4f lr=%.2e"
            " | kl=%.4f nll=%.4f gate=%.3f surprise=%.3f prog=%.3f err=%.3f topo=%.3f score=%.3f/%.3f"
            ...
            float(record.get("predict_loss", 0.0)),
            float(record.get("calibrate_loss", 0.0)),
            float(record.get("grad_norm", 0.0)),
            float(record.get("lr", 0.0)),
            float(record.get("kl_loss", 0.0)),
            float(record.get("nll_loss", 0.0)),
            float(record.get("gate_mean", 0.0)),
            float(record.get("surprise_mean", 0.0)),
            float(record.get("progress_mean", 0.0)),
            float(record.get("step_error_mean", 0.0)),
    ...
    def save_checkpoint(self, iteration: int, is_best: bool = False):
        ...
        ckpt_path = os.path.join(ckpt_dir, "ckpt.iter{}.pth".format(iteration))
        payload = {
        ...
        if is_best:
            torch.save(obj=payload, f=os.path.join(ckpt_dir, "best_val.pth"))

        max_keep = max(int(getattr(self._efes_cfg(), "max_keep_checkpoints", 3)), 1)
        ...
        while len(existing) > max_keep:
            ...
            os.remove(oldest)
            logger.info("Removed old EFESV3 checkpoint: %s", oldest)
    ...
    def train(self):
        ...
        logger.info("EFESV3 training starts...")
    ...
    def _train_interval(self, interval, ml_weight, sample_ratio):
        ...
        pbar = (
            tqdm.trange(interval, leave=False, dynamic_ncols=True, desc="EFESV3")
            if use_tqdm
            else range(interval)
        )
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        nll_loss_sum = torch.zeros((), device=self.device)
        calibrate_loss_sum = torch.zeros((), device=self.device)
        gate_sum = torch.zeros((), device=self.device)
        surprise_sum = torch.zeros((), device=self.device)
        progress_sum = torch.zeros((), device=self.device)
        step_error_sum = torch.zeros((), device=self.device)
        ...
        for stepk in range(self.max_len):
            ...
            step_outs = self.statenav_agent(
                ...
                prev_surprise=recurrent_state["prev_surprise"],
            ...
            surprise_sum = surprise_sum + step_outs["surprise_t"].sum()
            ...
            if mode == "eval" and active_eval_diag is not None:
                for i in range(self.envs.num_envs):
                    active_eval_diag[i]["surprise"].append(float(step_outs["surprise_t"][i].detach().item()))
            ...
            if mode == "train":
                ...
                kl_loss_sum = kl_loss_sum + step_outs["loss_micro"].sum()
                nll_loss_sum = nll_loss_sum + step_outs["loss_macro"].sum()
                teacher_valid = teacher_actions.ge(0)
                base_choice = nav_logits.argmax(dim=-1)
                error_target = (teacher_valid & base_choice.ne(teacher_actions)).to(dtype=avg_pano_embeds.dtype)
                step_error_sum = step_error_sum + error_target.sum()
                with cuda_autocast(enabled=False):
                    calib = F.binary_cross_entropy(
                        torch.sigmoid(step_outs["surprise_t"].float()).clamp(1e-4, 1.0 - 1e-4),
                        error_target.float(),
                        reduction="none",
                    )
            ...
            recurrent_state["prev_surprise"] = step_outs["surprise_t"]
            ...
            if mode == "eval":
                ...
                for i in range(self.envs.num_envs):
                    ...
                    if active_eval_diag is not None:
                        ...
                        metric["mean_surprise"] = self._mean_float_list(diag["surprise"])
        ...
        if mode == "train":
            ...
            nll_loss = nll_loss_sum / total_actions_f
            predict_loss = kl_loss + float(getattr(self._efes_cfg(), "lambda_macro", 1.0)) * nll_loss
            calibrate_loss = calibrate_loss_sum / total_actions_f
            total_loss = (
                nav_loss
                + float(getattr(self._efes_cfg(), "alpha", 0.5)) * predict_loss
                + float(getattr(self._efes_cfg(), "beta", 0.1)) * calibrate_loss
            )
            ...
            scalar_metrics = {
                ...
                "predict_loss": float(predict_loss.detach().item()),
            ...
            sum_metrics = {
                ...
                "surprise_sum": float(surprise_sum.detach().item()),
                "progress_sum": float(progress_sum.detach().item()),
                "step_error_sum": float(step_error_sum.detach().item()),
            ...
            self._last_step_metrics = {
                **scalar_metrics,
                ...
                "surprise_mean": float(sum_metrics["surprise_sum"]) / total_actions_global,
                "progress_mean": float(sum_metrics["progress_sum"]) / total_actions_global,
                "step_error_mean": float(sum_metrics["step_error_sum"]) / total_actions_global,
