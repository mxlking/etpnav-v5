...
mode="${2:-train}"
MASTER_PORT="${3:-29581}"
EXP_NAME="${4:-efes_v3_theory_r2r_${stage}}"
CKPT_PATH="${5:-}"
...
EXP_CONFIG="run_r2r/efes_v3_theory/efes_v3_theory_main.yaml"
STAGE_OPTS=()
case "$stage" in
    main)
        ;;
    short)
        STAGE_OPTS+=(IL.iters 6000)
        ;;
    no_predict)
        STAGE_OPTS+=(EFES_V3_THEORY.use_predictor False)
        ;;
    no_corrector)
        STAGE_OPTS+=(EFES_V3_THEORY.use_corrector False EFES_V3_THEORY.action_source etp)
        ;;
    no_body)
        STAGE_OPTS+=(EFES_V3_THEORY.use_body_loop False)
        ;;
    no_shifted)
        STAGE_OPTS+=(EFES_V3_THEORY.use_shifted_surprise False)
        ;;
    no_shared_mask)
        STAGE_OPTS+=(EFES_V3_THEORY.use_shared_valid_mask_only False)
        ;;
    no_safe_kl)
        STAGE_OPTS+=(EFES_V3_THEORY.use_safe_kl False)
        ;;
    no_dual_gate)
        STAGE_OPTS+=(EFES_V3_THEORY.use_dual_gate False)
        ;;
    no_mi)
        STAGE_OPTS+=(EFES_V3_THEORY.use_mi_proxy False)
        ;;
    *)
        echo "Unknown EFESV3Theory stage: $stage"
        echo "Valid stages: main short no_predict no_corrector no_body no_shifted no_shared_mask no_safe_kl no_dual_gate no_mi"
        exit 1
        ;;
esac
...
COMMON_LOGGING_ARGS=(
    ...
    EFES_V3_THEORY.LOGGING.step_log_every "$STEP_LOG_EVERY"
    EFES_V3_THEORY.LOGGING.use_tqdm "$USE_TQDM"
    EFES_V3_THEORY.LOGGING.write_step_metrics "$WRITE_STEP_METRICS"
    EFES_V3_THEORY.LOGGING.log_full_model_report_to_runtime "$LOG_FULL_MODEL_REPORT_TO_RUNTIME"
)
...
case "$mode" in
    ...
    eval)
        ...
        echo "[EFESV3Theory eval] using checkpoint: $CKPT_PATH"
        if [ "$NPROC" -eq 1 ]; then
    ...
    infer)
        ...
        echo "[EFESV3Theory infer] using checkpoint: $CKPT_PATH"
        if [ "$NPROC" -eq 1 ]; then
