        ...
        *,
        ...
        for batch_idx, current_vp in enumerate(current_vp_ids):
            ...
            if not bool(cooldown_ready[batch_idx].item()):
                continue
            if int(candidate_counts[batch_idx].item()) >= int(decision_candidate_min):
                update_mask[batch_idx] = True
                continue
            if (current_step_i - int(self.last_trigger_step[batch_idx].item())) >= int(timeout_steps):
                update_mask[batch_idx] = True
                continue
            if float(topo_novelty[batch_idx].item()) >= float(novelty_threshold):
                update_mask[batch_idx] = True
        return update_mask
