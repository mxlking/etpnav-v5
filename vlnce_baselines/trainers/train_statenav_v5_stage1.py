...
from vlnce_baselines.agents.statenav_agent_v5 import StateNavAgentV5
...
@baseline_registry.register_trainer(name="StateNavV5Stage1")
class StateNavV5Stage1Trainer(RLTrainer):
    stage_name = "Stage1"
    ...
    def _make_statenav_components(self) -> None:
        ...
        self.statenav_agent = StateNavAgentV5.from_config(
            self.config,
            x_dim=etp_dim,
            g_dim=etp_dim,
            lang_dim=etp_dim,
    ...
    def _configure_trainable_modules(self) -> None:
        ...
        if not bool(self.config.STATENAV.use_attention_transition):
            for param in state_model.attention_head.parameters():
                param.requires_grad_(False)
            for param in state_model.attn_health_scorer.parameters():
                param.requires_grad_(False)

        if bool(self.config.STATENAV.use_relation_state_extractor):
            # The repo-side V5 path only queries visual and graph tokens in
            # Phase 1. The legacy extra-token projection remains for older
            # rollout experiments, but it is inactive in the final v5.md path.
            for param in state_model.relational_state_extractor.extra_proj.parameters():
                param.requires_grad_(False)
            state_model.relational_state_extractor.extra_type.requires_grad_(False)
            state_model.relational_state_extractor.extra_pos.weight.requires_grad_(False)

            # Full V5 reads candidate curves from relation-conditioned states,
            # so the legacy h_post-only state projection / candidate head are
            # inactive and should not participate in DDP.
            if state_model.progress_curve_predictor.state_proj is not None:
                for param in state_model.progress_curve_predictor.state_proj.parameters():
                    param.requires_grad_(False)
            for param in state_model.progress_curve_predictor.cand_curve_head.parameters():
                param.requires_grad_(False)
        else:
            # When the relation extractor path is disabled, the full V5
            # relation-conditioned rollout branch is intentionally inactive.
            for module in (
                state_model.relational_state_extractor,
                state_model.unified_rollout_trunk,
                state_model.state_head,
            ):
                for param in module.parameters():
                    param.requires_grad_(False)

        if bool(self.config.STATENAV.use_relation_state_extractor) and not bool(
            self.config.STATENAV.use_open_eye_rollout
        ):
            for param in state_model.unified_rollout_trunk.parameters():
                param.requires_grad_(False)
    ...
    def _build_v4_video_debug_lines(
        ...
        *,
        ...
        line0 = (
            f"V5 step={int(step_index):02d} sel={int(selected_idx)} "
    ...
    def _prepare_step_metric_writer(self) -> None:
        ...
        if append_mode:
            ...
            if existing_header != expected_header:
                backup_path = f"{self._step_metrics_path}.legacy_before_v4_curve_training"
                backup_idx = 1
                while os.path.exists(backup_path):
                    backup_path = (
                        f"{self._step_metrics_path}.legacy_before_v4_curve_training.{backup_idx}"
                    )
                    backup_idx += 1
                shutil.copy2(self._step_metrics_path, backup_path)
                logger.warning(
                    "Existing V4 step metrics header mismatch. Backed up legacy file to %s "
                    "and restarting %s with updated curve-training columns.",
                    backup_path,
    ...
    def _log_step_metric_record(self, record: Dict[str, Any]) -> None:
        ...
        base_message = (
            "[step %06d/%06d | interval %03d/%03d] total=%.4f | plan=%.4f | kl=%.4f | curve=%.4f | attn=%.4f | "
            "valid=%.3f | grad=%.4f | lr=%.2e | attn_w=%.3f | adapter=%.3f | sample=%.4f | env/rank=%d | world=%d"
        )
        base_args: Tuple[Any, ...] = (
            ...
            float(record.get("curve_loss", 0.0)),
            float(record.get("attn_loss", 0.0)),
            float(record.get("curve_valid_ratio", 0.0)),
            float(record.get("grad_norm", 0.0)),
            float(record.get("lr", 0.0)),
            float(record.get("attn_warmup", 1.0)),
        ...
        if self._log_prior_post_metrics_enabled():
            logger.info(
                base_message
                + " | dt=%.3f dn=%.3f hs=%.3f st=%.3f hc=%.3f ch=%.3f ah=%.3f hb=%.3f tt=%.3f lv=%.2f tl=%.3f th=%.3f sc=%.2f ec=%.2f"
                + " | sec(step=%.2f roll=%.2f bw=%.2f opt=%.2f lang=%.2f wp=%.2f pano=%.2f nav=%.2f st=%.2f crpc=%.2f lrpc=%.2f env=%.2f prep=%.2f)",
                *base_args,
                float(record.get("d_t_mean", 0.0)),
                float(record.get("d_t_norm_mean", 0.0)),
                float(record.get("health_scalar_mean", 0.0)),
                float(record.get("s_t_norm_mean", 0.0)),
                float(record.get("curve_health_mean", 0.0)),
                float(record.get("cand_health_mean", 0.0)),
                float(record.get("attn_health_mean", 0.0)),
                float(record.get("health_bonus_mean", 0.0)),
                ...
                float(record.get("cand_real_pos_sec", 0.0)),
                float(record.get("label_rpc_sec", 0.0)),
    ...
    def _load_stage_checkpoint(self, checkpoint_path: str) -> int:
        ...
        if self.stage_name == "Stage1" and not self.config.IL.is_requeue and not has_statenav_state:
            if checkpoint_realpath != _RELEASE_R2R_CKPT:
                raise ValueError(
                    "Fresh Stage1 experiments in ETPNav_selected must start from "
    ...
    def _initialize_policy(
        ...
        if self.stage_name == "Stage1" and not load_from_ckpt and not config.IL.is_requeue:
            raise ValueError(
                "StateNav Stage1 must start from a trained ETPNav navigation checkpoint. "
    ...
    def train(self):
        ...
        logger.info("StateNav V5 %s training starts...", self.stage_name)
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        trace_eval = mode == "eval" and self._v4_trace_enabled()
        ...
        attn_loss_sum = torch.zeros((), device=self.device)
        ...
        label_contexts = self._init_label_contexts() if mode == "train" else None
        trace_label_contexts = self._init_label_contexts() if trace_eval else None
        active_curve_sequences = (
            [{"pred_curve": [], "progress_ratio": [], "valid": []} for _ in range(self.envs.num_envs)]
            if mode == "train"
            else None
        )
        ...
        prev_gt_predicted_attn = [None for _ in range(self.envs.num_envs)] if mode == "train" else None
        finalized_curve_sequences: List[Dict[str, Tensor]] = []
        ...
        for stepk in range(self.max_len):
            ...
            if mode == "eval":
                for i in range(self.envs.num_envs):
                    active_eval_self_stats[i]["d_t"].append(float(d_t_step[i].detach().item()))
                    active_eval_self_stats[i]["health_scalar"].append(
                        float(health_scalar_step[i].detach().item())
                    )
                    active_eval_self_stats[i]["curve_health"].append(
                        float(curve_health_step[i].detach().item())
                    )
                    active_eval_self_stats[i]["cand_health"].append(
                        float(cand_health_step[i].detach().item())
                    )
                    active_eval_self_stats[i]["health_bonus"].append(
                        float(health_bonus_step[i].detach().item())
                    )
                    ...
                    active_eval_self_stats[i]["attn_health"].append(
                        float(attn_health_step[i].detach().item())
                    )
                    active_eval_self_stats[i]["s_t_norm"].append(
                        float(s_t_norm_step[i].detach().item())
                    )
            ...
            if mode == "train":
                ...
                teacher_safe = teacher_actions.clamp(min=0, max=nav_outs["gmap_embeds"].size(1) - 1)
                teacher_valid = (teacher_actions >= 0) & (teacher_actions < nav_outs["gmap_embeds"].size(1))
                if bool(self.config.STATENAV.use_attention_transition):
                    for i in range(self.envs.num_envs):
                        if prev_gt_predicted_attn[i] is not None:
                            # attn_weights_post is the next-step token distribution [B, T]
                            # produced by SelfRSSMCorrection, not the language glimpse vector.
                            attn_loss_sum = attn_loss_sum + attention_transition_loss(
                                predicted_attn=prev_gt_predicted_attn[i].unsqueeze(0),
                                target_attn=step_outs["attn_weights_post"][i].detach().unsqueeze(0),
                                lang_mask=txt_masks[i].unsqueeze(0),
                                reduction="sum",
                            )
                            total_attn_pairs += 1

                    gt_predicted_attn = step_outs["predicted_attn"][
                        torch.arange(self.envs.num_envs, device=self.device),
                        teacher_safe,
                    ]
                    for i in range(self.envs.num_envs):
                        prev_gt_predicted_attn[i] = (
                            gt_predicted_attn[i].clone() if bool(teacher_valid[i].item()) else None
                        )
                else:
                    for i in range(self.envs.num_envs):
                        prev_gt_predicted_attn[i] = None
                ...
                state_valid_mask = []
                progress_ratio = []
                label_rpc_start = self._timing_now()
                ref_distances_batch = self.envs.call(
                    ["current_dist_to_refpath"] * self.envs.num_envs,
                    [{"path": getattr(context, "reference_path_list", getattr(context, "reference_path", getattr(context, "ref_path", None)))} for context in label_contexts],
                )
                timing_sums["label_rpc_sec"] += self._timing_now() - label_rpc_start
                for i in range(self.envs.num_envs):
                    ref_distances = ref_distances_batch[i]
                    label_top_candidate_id = self._select_label_top_candidate_id(
                        teacher_action=int(teacher_actions[i].item()),
                        raw_etp_logits=nav_logits[i],
                        candidate_mask=candidate_mask[i],
                    )
                    label_build_start = self._timing_now()
                    label = self.label_builder.build_step(
                        context=label_contexts[i],
                        current_position=cur_pos[i],
                        current_heading=heading_from_quaternion(np.array(cur_ori[i])),
                        node_id=cur_vp[i],
                        top_candidate_id=label_top_candidate_id,
                        ref_distances=ref_distances,
                    )
                    timing_sums["label_build_sec"] += self._timing_now() - label_build_start
                    state_valid_mask.append(label["state_valid_mask"])
                    progress_ratio.append(label["progress_ratio"])

                state_valid_tensor = torch.tensor(state_valid_mask, device=self.device, dtype=torch.long)
                total_valid_states += int(state_valid_tensor.sum().item())

                for i in range(self.envs.num_envs):
                    gt_idx = int(teacher_safe[i].item())
                    gt_curve_pred = step_outs["cand_progress_curves"][i, gt_idx]
                    is_valid_teacher = bool(teacher_valid[i].item()) and not bool(candidate_mask[i, gt_idx].item())
                    active_curve_sequences[i]["pred_curve"].append(gt_curve_pred)
                    active_curve_sequences[i]["progress_ratio"].append(float(progress_ratio[i]))
                    active_curve_sequences[i]["valid"].append(
                        float(state_valid_tensor[i].item()) * float(is_valid_teacher)
                    )
            ...
            if mode == "eval":
                ...
                for i in range(self.envs.num_envs):
                    ...
                    if self._log_prior_post_eval_enabled():
                        ...
                        metric["mean_d_t"] = self._mean_float_list(self_stats["d_t"])
                        metric["mean_health_scalar"] = self._mean_float_list(self_stats["health_scalar"])
                        metric["mean_s_t_norm"] = self._mean_float_list(self_stats["s_t_norm"])
                        metric["mean_curve_health"] = self._mean_float_list(self_stats["curve_health"])
                        metric["mean_cand_health"] = self._mean_float_list(self_stats["cand_health"])
                        metric["mean_health_bonus"] = self._mean_float_list(self_stats["health_bonus"])
                        ...
                        metric["mean_attn_health"] = self._mean_float_list(self_stats["attn_health"])
                    ...
                    if self._log_prior_post_eval_enabled():
                        running_means.update(
                            {
                                "mean_d_t": sum(v["mean_d_t"] for v in self.stat_eps.values()) / evaluated_eps,
                                "mean_health_scalar": sum(v["mean_health_scalar"] for v in self.stat_eps.values()) / evaluated_eps,
                                "mean_s_t_norm": sum(v["mean_s_t_norm"] for v in self.stat_eps.values()) / evaluated_eps,
                                "mean_curve_health": sum(v["mean_curve_health"] for v in self.stat_eps.values()) / evaluated_eps,
                                "mean_cand_health": sum(v["mean_cand_health"] for v in self.stat_eps.values()) / evaluated_eps,
                                "mean_health_bonus": sum(v["mean_health_bonus"] for v in self.stat_eps.values()) / evaluated_eps,
                                ...
                                "mean_attn_health": sum(v["mean_attn_health"] for v in self.stat_eps.values()) / evaluated_eps,
                    ...
                    if self._log_prior_post_eval_enabled():
                        ep_done_message += (
                            f" | dt={metric['mean_d_t']:.3f}"
                            f" hs={metric['mean_health_scalar']:.3f}"
                            f" st={metric['mean_s_t_norm']:.3f}"
                            f" hc={metric['mean_curve_health']:.3f}"
                            f" ch={metric['mean_cand_health']:.3f}"
                            f" ah={metric['mean_attn_health']:.3f}"
                            f" hb={metric['mean_health_bonus']:.3f}"
                            f" tt={metric['mean_dt_temperature']:.3f}"
                            f" dn={metric['mean_d_t_norm']:.3f}"
                    ...
                    if self.local_rank < 1 and (
                        ...
                        if self._log_prior_post_eval_enabled():
                            eval_live_message += (
                                f" dt={running_means['mean_d_t']:.3f}"
                                f" hs={running_means['mean_health_scalar']:.3f}"
                                f" st={running_means['mean_s_t_norm']:.3f}"
                                f" hc={running_means['mean_curve_health']:.3f}"
                                f" ch={running_means['mean_cand_health']:.3f}"
                                f" ah={running_means['mean_attn_health']:.3f}"
                                f" hb={running_means['mean_health_bonus']:.3f}"
                                f" tt={running_means['mean_dt_temperature']:.3f}"
                                f" dn={running_means['mean_d_t_norm']:.3f}"
            ...
            if done_indices:
                if mode == "train":
                    for i in done_indices:
                        seq = active_curve_sequences[i]
                        if seq["pred_curve"]:
                            finalized_curve_sequences.append(
                                {
                                    "pred_curve": torch.stack(seq["pred_curve"], dim=0),
                                    "progress_ratio": torch.tensor(
                                        seq["progress_ratio"],
                ...
                for i in reversed(done_indices):
                    ...
                    if mode == "train":
                        active_curve_sequences.pop(i)
                        label_contexts.pop(i)
                    if mode == "eval":
                        active_eval_self_stats.pop(i)
                    if mode == "train":
                        prev_gt_predicted_attn.pop(i)
                    if trace_eval:
        ...
        if mode == "train":
            self._finalize_curve_sequences(active_curve_sequences, finalized_curve_sequences)
            curve_loss, total_valid_steps = self._compute_progress_curve_loss(finalized_curve_sequences)
            plan_loss = plan_loss_sum / max(total_actions, 1)
            kl_loss = kl_loss_sum / max(total_actions, 1)
            attn_loss = attn_loss_sum / max(total_attn_pairs, 1)
            attn_warmup = self._compute_attn_warmup()
            total_loss = (
                plan_loss
                + float(self.config.STATENAV.lambda_kl) * self._compute_kl_warmup() * kl_loss
                + float(self.config.STATENAV.lambda_curve) * curve_loss
                + float(self.config.STATENAV.lambda_attn) * attn_warmup * attn_loss
            )
            ...
            scalar_metrics = {
                ...
                "curve_loss": float(curve_loss.detach().item()),
            ...
            count_metrics = {
                ...
                "total_valid_states": float(total_valid_steps),
                "total_attn_pairs": float(total_attn_pairs),
            }
            ...
            self._last_step_metrics = {
                **scalar_metrics,
                "curve_valid_ratio": float(count_metrics["total_valid_states"]) / total_actions_global,
                "total_actions": float(count_metrics["total_actions"]),
                "total_valid_states": float(count_metrics["total_valid_states"]),
                "total_attn_pairs": float(count_metrics["total_attn_pairs"]),
                "kl_warmup": float(self._compute_kl_warmup()),
                "attn_warmup": float(attn_warmup),
            ...
            if self._log_prior_post_metrics_enabled():
                self._last_step_metrics.update(
                    {
                        "d_t_mean": float(intervention_sum_metrics["d_t_sum"]) / total_actions_global,
                        "d_t_norm_mean": float(intervention_sum_metrics["d_t_norm_sum"]) / total_actions_global,
                        "health_scalar_mean": float(intervention_sum_metrics["health_scalar_sum"]) / total_actions_global,
                        "s_t_norm_mean": float(intervention_sum_metrics["s_t_norm_sum"]) / total_actions_global,
                        "curve_health_mean": float(intervention_sum_metrics["curve_health_sum"]) / total_actions_global,
                        "cand_health_mean": float(intervention_sum_metrics["cand_health_sum"]) / total_actions_global,
                        "attn_health_mean": float(intervention_sum_metrics["attn_health_sum"]) / total_actions_global,
                        "health_bonus_mean": float(intervention_sum_metrics["health_bonus_sum"]) / total_actions_global,
            ...
            self.logs["kl_loss"].append(self._last_step_metrics["kl_loss"])
            self.logs["curve_loss"].append(self._last_step_metrics["curve_loss"])
            self.logs["attn_loss"].append(self._last_step_metrics["attn_loss"])
            self.logs["curve_valid_ratio"].append(self._last_step_metrics["curve_valid_ratio"])
            self.logs["kl_warmup"].append(self._last_step_metrics["kl_warmup"])
            self.logs["attn_warmup"].append(self._last_step_metrics["attn_warmup"])
            self.logs["adapter_warmup"].append(self._last_step_metrics["adapter_warmup"])
            if self._log_prior_post_metrics_enabled():
                self.logs["d_t_mean"].append(self._last_step_metrics["d_t_mean"])
                self.logs["d_t_norm_mean"].append(self._last_step_metrics["d_t_norm_mean"])
                self.logs["health_scalar_mean"].append(self._last_step_metrics["health_scalar_mean"])
                self.logs["s_t_norm_mean"].append(self._last_step_metrics["s_t_norm_mean"])
                self.logs["curve_health_mean"].append(self._last_step_metrics["curve_health_mean"])
                self.logs["cand_health_mean"].append(self._last_step_metrics["cand_health_mean"])
                self.logs["attn_health_mean"].append(self._last_step_metrics["attn_health_mean"])
                self.logs["health_bonus_mean"].append(self._last_step_metrics["health_bonus_mean"])
