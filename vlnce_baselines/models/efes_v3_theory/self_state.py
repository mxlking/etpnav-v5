...
class TheorySelfState(nn.Module):
    def __init__(
        self,
        d_model: int = 768,
        d_obs: int = 768,
        ...
        self.d_instr = int(d_instr)
        self.use_body_loop = bool(use_body_loop)
        ...
        self.instr_proj = nn.Linear(self.d_instr, self.d_model)
    ...
    def forward(
        ...
        if prev_self.size(-1) != self.d_model:
            raise ValueError("TheorySelfState expected prev_self dim {}, got {}".format(self.d_model, prev_self.size(-1)))
        if obs_feat.size(-1) != self.d_obs:
            raise ValueError("TheorySelfState expected obs dim {}, got {}".format(self.d_obs, obs_feat.size(-1)))
        if instr_tokens.size(-1) != self.d_instr:
            raise ValueError("TheorySelfState expected instruction dim {}, got {}".format(self.d_instr, instr_tokens.size(-1)))

        if not self.use_body_loop:
            prev_u_tilde = torch.zeros_like(prev_u_tilde)

        body_token = self.body_proj(
            torch.cat(
                [
                    self._as_column(prev_u_tilde),
        ...
        if instr_mask is not None:
            ...
            extra_valid = torch.ones(instr_mask.size(0), 4, device=instr_mask.device, dtype=torch.bool)
